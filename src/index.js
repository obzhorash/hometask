...
/*
 ...
 Пример:
   createDivWithText('loftschool') // создаст элемент div, поместит в него
   'loftschool' и вернет созданный элемент
 */
...
/*
 ...
 Пример:
   ...
   элементами div и span
   т.к. следующим соседом этих элементов является элемент с тегом P
 */
function findAllPSiblings(where) {

    let childrenElement = where.children;

    let array = [];

    for (let i = 0; i < childrenElement.length-1; i++) {

        if ( childrenElement[i].nextElementSibling.tagName === 'P' ) {
            array.push(childrenElement[i])
        }
    }
...
/*
 ...
 Функция должна перебрать все дочерние узлы элемента переданного в параметре
 where и удалить из него все текстовые узлы
    ...
    let observer = new MutationObserver( function( mutations ) {  
        ...
        mutations.forEach( function( mutation ) {
            ...
            if ( mutation.addedNodes.length ) {
                objMutations.add.nodes.push(...mutation.addedNodes);                
            } else {
                objMutations.remove.nodes.push(...mutation.removedNodes);
            }
        });  

        if ( objMutations.add.nodes.length ) {
            fn( objMutations.add );
        }

        if ( objMutations.remove.nodes.length ) {
            fn( objMutations.remove );
        }
